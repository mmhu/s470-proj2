...
We are not adding any interaction because sports and gender interaction still gives you the straightish line for fitted plots. Plus, we believe sports themselves soley determine the injury issues no matter gender. In terms of the residual and fitted, the loess fitting is doing a good job capturing the residual pattern which does not seem to deviate much from 0.
...
Here we have overdispersion value as `r Overdispersion`, which is over 1 but pretty close to 1. Thus, this poisson model with the data is arguably correct. Also, the poisson model does not allow variance to be adjusted independently of the mean.
...
We basically see When comparing the two cases where Lacrosse or Gynmastics is the baseline, we have a a better idea of ratio relative to the baseline to use Lacrosse rather than Gynmastics because it is clear to detect the 95% CI indicates narrower widths 
